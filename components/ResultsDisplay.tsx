...
const ResultCard: React.FC<ResultCardProps> = ({ label, value, highlight = false }) => (
  <div className={`p-4 rounded-lg flex-1 text-center ${highlight ? 'bg-[#4F772D] text-white' : 'bg-slate-100 text-slate-800'}`}>
    <p className={`text-sm ${highlight ? 'text-green-100' : 'text-slate-600'}`}>{label}</p>
    <p className={`text-2xl font-bold ${highlight ? 'text-white' : 'text-[#4F772D]'}`}>{formatCurrency(value)}</p>
  </div>
);
...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ result }) => {
  return (
    <div className="bg-white p-6 md:p-8 rounded-lg shadow-md mt-10 space-y-8">
      <h2 className="text-2xl font-bold text-[#4F772D]">Resultado</h2>
      ...
      <div>
          ...
          <div className="flex items-center justify-center gap-6 mb-4 text-sm">
            <div className="flex items-center">
                <span className="w-4 h-4 rounded-full bg-[#4F772D] mr-2"></span>
